...
def _build_engagement_timeline(
    discovery_dir: Path,
    planning_dir: Path,
    testing_dir: Path,
    ...
    stages = [
        _timeline_stage("Discovery", _timestamps_from_files(discovery_dir / "events.json", discovery_dir / "memory.json")),
        _timeline_stage(
            "Security test planning",
            _timestamps_from_files(planning_dir / "events.json", planning_dir / "memory.json"),
        ),
        _timeline_stage(
            "Security testing",
            _timestamps_from_files(testing_dir / "events.json", testing_dir / "memory.json")
            + [_text(test.get("executed_at")) for test in executed_tests if isinstance(test, dict)],
        ),
        _timeline_stage(
            "Final reporting",
            _timestamps_from_files(final_report_dir / "events.json", final_report_dir / "memory.json"),
        ),
    ]
    present_stages = [stage for stage in stages if stage["started_at"] or stage["completed_at"]]
    timestamps: list[str] = []
    for stage in present_stages:
        timestamps.extend([_text(stage.get("started_at")), _text(stage.get("completed_at"))])
...
def _timeline_stage(name: str, timestamps: list[str]) -> dict[str, str]:
    timestamps = [timestamp for timestamp in timestamps if timestamp]
    return {
        "name": name,
        "started_at": min(timestamps) if timestamps else "",
...
def _timestamps_from_files(*paths: Path) -> list[str]:
    timestamps: list[str] = []
    for path in paths:
